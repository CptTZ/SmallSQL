...
public class BasicTestCase {
    ...
    public void assertRSMetaData(ResultSet rs, String[] colNames, int[] types) throws Exception {
        ...
        for (int i = 1; i <= count; i++) {
            assertEquals(colNames[i - 1], rm.getColumnName(i));
            assertEquals(colNames[i - 1], rm.getColumnLabel(i));
    ...
    void assertEqualsRsValue(Object obj, ResultSet rs, boolean needTrim) throws Exception {
        ...
        if (obj instanceof Time) {
            ...
            assertEquals(obj.toString(), rs.getString(name));
        }
        ...
        if (obj instanceof Number) {
            ...
            if (!needTrim) {
                assertEquals(valueStr, rs.getString(name));
            }
     ...
     *
     * @param ordinal month ordinal (1-based).
